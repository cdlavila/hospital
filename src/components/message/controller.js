...
class Controller {
  ...
  static async updateMessage(req, res) {
    try {
      if (!req?.params?.id || !req?.body?.message) return Response.error(res, statusCode?.BAD_REQUEST, 'Message id and message are required');
  ...
  static async deleteMessage(req, res) {
    try {
      if (!req?.params?.id) return Response.error(res, statusCode?.BAD_REQUEST, 'Message id is required');
